from quat_multiply import quat_multiply, quat_multiply_batch
import numpy as np
...
def rotate_vector_batch(vin_batch, q_batch):
    ...
    vin_transformed_batch = np.hstack(
        (vin_batch.copy(), np.zeros((vin_batch.shape[0], 1))))
    return quat_multiply_batch(
        quat_multiply_batch(
            q_batch, vin_transformed_batch), q_transformed_batch
    )[:, 0:3]
